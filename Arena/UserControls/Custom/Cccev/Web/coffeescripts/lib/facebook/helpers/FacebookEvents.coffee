...
facebookEvents.defaults =
	onLoginSuccess: (result) ->
		if result.d > -1 then window.location = CentralAZ.Facebook.Helpers.Authentication.config.redirectPath
	...
	FB.getLoginStatus (response) ->
		...
		if response.status is 'connected' then facebookEvents.trigger 'facebookLoggingIn', response
			...
			FB.login (response) ->
				...
				# The user has denied CentralAZ app access to their info
